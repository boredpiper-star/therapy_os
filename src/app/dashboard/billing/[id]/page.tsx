...
import { format, endOfMonth, parse, startOfMonth } from "date-fns"
...
export default async function InvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // 2. Fetch the specific completed sessions for this month to create line items
  const dateStr = `${invoice.billing_year}-${invoice.billing_month.toString().padStart(2, '0')}-01`
  ...
  const { data: sessions } = await supabase
    .from('appointments')
    .select('*, therapist:therapist_id(full_name)')
  ...
  const billingPeriod = format(parse(startDate, 'yyyy-MM-dd', new Date()), 'MMMM yyyy')
  const issueDate = format(new Date(invoice.created_at), 'MMMM dd, yyyy')

  // Identify the Parent Name (Fallback to Mother, or default text if both missing)
  const parentName = invoice.patient.father_name || invoice.patient.mother_name || "Parent / Guardian";

  // Helper to format the service name
  const getServiceName = (type: string) => {
    const t = type?.toLowerCase() || "";
    if (!t || t === "single" || t === "series") {
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 sm:p-8 font-sans">
      ...
      {/* ACTION BAR (Hidden when printing via Tailwind 'print:hidden') */}
      ...
      <div id="printable-invoice" className="max-w-4xl mx-auto bg-white shadow-xl rounded-lg border border-slate-200 p-8 sm:p-12 relative">
        ...
        <div className="flex justify-between items-start border-b-2 border-slate-100 pb-5 mb-5 relative z-10 gap-4">
          ...
          {/* LEFT SIDE: CLINIC LETTERHEAD BLOCK */}
          <div className="flex flex-col gap-1.5 max-w-[600px]">
            <div className="flex items-center gap-3">
              <div className="w-16 h-16 sm:w-20 sm:h-20 shrink-0 flex items-center justify-center bg-white">
                ...
                <img 
                  src="/logo.png" 
                  alt="AP Paediatric Therapy Services Logo" 
            ...
            <div className="text-[13px] text-slate-500 leading-[1.5] mt-1">
              ...
              <p className="mt-1.5 font-semibold text-slate-700">
                UDYAM Reg No: UDYAM-KR-03-0464365
              </p>
            </div>
          </div>
        ...
        {/* LINE ITEMS TABLE - S.No. & Duration Added */}
          ...
          <table className="w-full text-left border-collapse border border-slate-300">
            <thead className="bg-slate-50">
              <tr className="text-sm">
                ...
                <th className="py-2.5 px-4 border border-slate-300 font-bold text-slate-900 w-32">Date</th>
            ...
            <tbody className="text-sm">
              {sessions?.map((session, index) => (
                <tr key={session.id} className="text-slate-700 hover:bg-slate-50/50">
                  <td className="py-3 px-4 border border-slate-300 text-center font-medium">{index + 1}</td>
                  <td className="py-3 px-4 border border-slate-300 font-medium">{format(parse(session.date, 'yyyy-MM-dd', new Date()), 'dd/MM/yyyy')}</td>
                  <td className="py-3 px-4 border border-slate-300">
                    <p className="font-medium text-slate-900">{getServiceName(session.type)}</p>
                  ...
                  <td className="py-3 px-4 border border-slate-300 text-right font-medium text-slate-900">
                    ₹{invoice.rate_per_session.toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex justify-end mb-8 relative z-10">
          <div className="w-64 space-y-2.5 pr-2 pt-2">
            <div className="flex justify-between text-sm text-slate-600">
              <span>Subtotal ({invoice.session_count} sessions)</span>
